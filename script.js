document.addEventListener('DOMContentLoaded', () => {
    ...
    if (timelineSection && orbitPath && energyDot && orbitContainer) {
        ...
        const startRevealSequence = () => {
            ...
            timelineCards.forEach((card, index) => {
                const revealTime = index * (isMobile ? 300 : staggerDelay);
        ...
        // Observe timeline section - trigger at 10% visibility
    ...
    if (countdownDays && countdownHours && countdownMin && countdownSec) {
        // Target date: March 7, 2026 — 08:00 AM local time
        const targetDate = new Date('March 7, 2026 08:00:00').getTime();
    ...
    // ==========================================
    // ==========================================
    // 7. Prize Calculator (Simplified Logic)
    // ==========================================
    ...
    if (teamSizeSlider && teamSizeValue && totalInvestment) {

        const updateCalculator = () => {
            // 1. Get current slider value
            const teamSize = parseInt(teamSizeSlider.value);

            // 2. Calculate Total Investment (₹200 per person)
            const total = teamSize * 200;
